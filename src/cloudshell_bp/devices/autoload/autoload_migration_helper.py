...
def migrate_autoload_details(autoload_details, shell_name, shell_type):
    ...
    for attribute in autoload_details.attributes:
        ...
        if not attribute.relative_address:  # Root element
            attribute.attribute_name = "{shell_type}.{attr_name}".format(
                shell_type=shell_type, attr_name=attribute.attribute_name
            )
        else:
            attribute.attribute_name = "{model}.{attr_name}".format(
                model=mapping[attribute.relative_address], attr_name=attribute.attribute_name
            )
