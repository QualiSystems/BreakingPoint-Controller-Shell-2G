...
from cloudshell_bp.core.driver_response import DriverResponse
from cloudshell_bp.core.driver_response_root import DriverResponseRoot
from cloudshell_bp.devices.json_request_helper import JsonRequestDeserializer
from cloudshell_bp.devices.networking_utils import (
    command_logging,
    serialize_to_json,
    validate_vlan_number,
    validate_vlan_range,
)
from cloudshell_bp.devices.runners.interfaces.connectivity_runner_interface import ConnectivityOperationsInterface
from cloudshell_bp.networking.apply_connectivity.models.connectivity_result import (
...
class ConnectivityRunner(ConnectivityOperationsInterface):
            ...
            if action.type == "setVlan":
                ...
                for vlan_id in self._get_vlan_list(action.connectionParams.vlanId):
                    add_vlan_thread = Thread(
                        target=self.add_vlan, name=action_id, args=(vlan_id, full_name, port_mode, qnq, ctag)
                    )
                    add_vlan_thread_list.append(add_vlan_thread)
            elif action.type == "removeVlan":
                for vlan_id in self._get_vlan_list(action.connectionParams.vlanId):
                    remove_vlan_thread = Thread(
                        target=self.remove_vlan,
                        name=action_id,
                        args=(
                            vlan_id,
                            full_name,
        ...
        for action in holder.driverRequest.actions:
            ...
            if all(result_statuses):
                action_result = ConnectivitySuccessResponse(
                    action, "Add Vlan {vlan} configuration successfully completed".format(vlan=action.connectionParams.vlanId)
                )
            else:
                message_details = "\n\t".join(message)
                action_result = ConnectivityErrorResponse(
                    ...
                    "\nAdd Vlan configuration details:\n{message_details}".format(
                        vlan=action.connectionParams.vlanId, message_details=message_details
                    ),
                )
    ...
    def _validate_request_action(self, action):
        ...
        if is_fail:
            raise Exception(
                ...
                "Mandatory field {0} is missing in ApplyConnectivityChanges request json".format(fail_attribute),
            )
    ...
    def add_vlan(self, vlan_id, full_name, port_mode, qnq, c_tag):
        ...
        try:
            action_result = self.add_vlan_flow.execute_flow(
                vlan_range=vlan_id, port_mode=port_mode, port_name=full_name, qnq=qnq, c_tag=c_tag
            )
    ...
    def remove_vlan(self, vlan_id, full_name, port_mode):
        ...
        try:
            ...
            action_result = self.remove_vlan_flow.execute_flow(vlan_range=vlan_id, port_name=full_name, port_mode=port_mode)
            self.result[current_thread().name].append((True, action_result))
        except Exception as e:
            self._logger.error(traceback.format_exc())
            self.result[current_thread().name].append((False, e.message))
